...
| Versión | 1.1 |
| Subtarea | ST-02 de la reformulación SDD 2.1D |
| Fecha | 2026-06-08 |
...
- Resultado: la salida `/SDD2.1D/docs/` es idéntica a la que produce el template hoy para ese tipo. ST-09 verificará esta equivalencia como prueba de no regresión contra la línea base de ST-01.
...
D-C. Ubicación del workspace: CONFIRMADA. Los entregables de la reformulación viven en `/SDD2.1D/devs/_reformulacion/` (espejo de `_bootstrap/`). El `SOLUTION-MANIFEST-template.md` que produce ST-03 va a `/SDD2.1D/devs/intake/` como indica el prompt.
