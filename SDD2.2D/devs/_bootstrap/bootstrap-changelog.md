# Changelog del bootstrap SDD 2.2
...
El bootstrap del template SDD 2.2 produjo, en 6 fases (F0 a F5) y a través de aproximadamente 20 despachos de subagentes coordinados por un master-prompt de bootstrap, un template metodológico stack-agnóstico orientado a la construcción de proyectos por especificación viva, basado en la auditoría del fuente SDD 1.0 (caso de estudio histórico "Motor DSL" de la cátedra de Aplicada 2026).

Se generaron 23 archivos markdown distribuidos en seis carpetas funcionales: `/SDD2.2D/devs/_bootstrap/`, `/SDD2.2D/devs/intake/`, `/SDD2.2D/devs/rules/`, `/SDD2.2D/devs/orchestrator/`, `/SDD2.2D/devs/guides/`, `/SDD2.2D/guides/`. La carpeta `/SDD2.2D/docs/` queda reservada vacía con `.gitkeep` como destino del orquestador del proyecto.
...
| Métrica | Valor |
|---|---|
| Archivos generados en `/SDD2.2D/` | 23 markdown + 1 `.gitkeep` |
...
### /SDD2.2D/devs/orchestrator/

| Archivo | Líneas | Propósito |
|---|---|---|
| `master-prompt.md` | 621 | Prompt orquestador autocontenido del proyecto. Despacha la generación de `/SDD2.2D/docs/` por fases A–H trazables a la cadena D6, con auditoría entre fases. |

### /SDD2.2D/devs/guides/

| Archivo | Líneas | Propósito |
|---|---|---|
| `marco-teorico-sdd_v1.0.md` | 1.747 | Marco teórico SDD 2.2: 14 capítulos. Spec-First / Spec-Anchored / Spec-as-Source, cadena D6, 8 tipos D8, ADRs, calidad, DevOps, prompting colaborativo, bibliografía. |
...
1. Validar el template ejecutándolo en un proyecto piloto real, completando ambas plantillas de intake y disparando el `master-prompt.md` para generar `/SDD2.2D/docs/`.
