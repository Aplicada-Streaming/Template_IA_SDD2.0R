# Matriz de coherencia del template SDD 2.2D — Línea base (ST-01)

| Campo | Valor |
|---|---|
| Archivo | `matriz-coherencia-template_v1.0.md` |
| Versión | 1.0 |
| Subtarea | ST-01 de la reformulación SDD 2.2D (solución más jerarquía de proyectos) |
| Fecha | 2026-06-08 |
...
| Alcance | Todos los markdown de `/SDD2.2D/devs/**`, `/SDD2.2D/guides/**` y `/SDD2.2D/docs/**` |

---

## §1 Resumen ejecutivo

Se inventariaron y evaluaron 25 documentos markdown del template contra su objetivo declarado, antes de introducir cualquier cambio. La carpeta `/SDD2.2D/docs/` está vacía (solo `.gitkeep`): es el destino de salida del orquestador, no contiene artefactos a evaluar.
...
Evidencia: el texto sobre «Spec-First» dice que la especificación se «descarta» tras la implementación, y luego propone «Spec-Anchored» como estándar de SDD 2.2.
...
Evidencia: el archivo se llama `guia-usuario-sdd2.0_v1.0.md` y su metadato `Documento` repite «sdd2.0», mientras el cuerpo, el repositorio (`SDD2.2D`) y el commit de rename declaran «SDD 2.2». El `bootstrap-changelog.md` y `audit-fase-4.md` citan el nombre tal cual, reflejando el desfase.
Lectura: resto del rename de `sdd2.0` a `SDD2.2D` que no alcanzó el nombre de este archivo. No bloquea operación; es consistencia de marca.
Recomendación: renombrar a `guia-usuario-sdd2.2_v1.0.md` (con archivado del anterior según D5) y actualizar el metadato y las referencias internas. Atendible en ST-08 o como tarea de higiene. Nota: `marco-teorico-sdd_v1.0.md` no lleva marca de versión en el nombre, así que no requiere rename.
Estado: RESUELTO el 2026-06-10 (tarea de higiene posterior al cierre de ST-09). El archivo se renombró a `guia-usuario-sdd2.2_v1.0.md` con `git mv`; se actualizaron el metadato `Documento`, la mención «novedades de la 2.0» a «2.1» y las referencias internas del árbol. Nota: los registros históricos de `_bootstrap/` (changelog, audit-fase-4) siguen citando el nombre anterior por ser registros congelados del bootstrap.
